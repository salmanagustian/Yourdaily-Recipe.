    ...
    <div class="mt-8">
        ...
        <div class="flex flex-col items-center md:flex-row my-10">
            <div class="mr-auto">
                <div
                  ...
                  v-if="loadingRecipe"></div>
                <div v-else class="text-2xl font-semibold text-truGray-700 dark:text-white/80">Daftar Resep</div>
            </div>
            <div>
                ...
                <div v-else class="flex items-center w-[430px] rounded-xl bg-truGray-100 dark:bg-[#2e2e33] py-3 px-5">
        ...
        <div class="flex mb-10" v-if="resultRecipe && resultRecipeLength > 0">
           <div class="text-truGray-600 text-base font-medium">
              Ada {{ resultRecipeLength }} data resep loh.. untuk pencarian <span class="text-amber-400 text-base">"{{ searchRecipe }}"</span>
           </div>
        </div>
        <div class="flex flex-col mb-10" v-else-if="resultRecipe && resultRecipeLength == 0">
           <div class="text-truGray-600 text-base font-medium">
              Maaf, sepertinya kami tidak menemukan resep untuk pencarian <span class="text-amber-400 text-base">"{{ searchRecipe }}"</span>
           </div>
           <div class="text-truGray-600 text-base font-medium">
        ...
        <div v-else class="grid grid-cols-1 gap-8 md:grid-cols-4">
            <div class="recipes-food" v-for="(recipe, index) in recipes" :key="index">
                ...
                <div class="recipes-food__title">
                   <router-link v-bind:to="`/detail/${recipe.key}`">{{ recipe.title }}</router-link>
                </div>
